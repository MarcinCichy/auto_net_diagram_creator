...
try:
    ...
    class snmp_utils: # type: ignore [no-redef]
        ...
        # (Te same co w discovery.py, można by je przenieść do wspólnego miejsca, jeśli to problem)
        @staticmethod
        def snmp_get_lldp_neighbors(h: str, c: str, timeout: int = 0, retries: int = 0) -> Optional[List[Tuple[int, str, str]]]: return None
        @staticmethod
        def snmp_get_cdp_neighbors(h: str, c: str, timeout: int = 0, retries: int = 0) -> Optional[List[Tuple[int, str, str]]]: return None
        @staticmethod
        def snmp_get_bridge_baseport_ifindex(h: str, c: str, timeout: int = 0, retries: int = 0) -> Optional[Dict[int, int]]: return None
        @staticmethod
        def snmp_get_fdb_entries(h: str, c: str, timeout: int = 0, retries: int = 0) -> Optional[List[Tuple[str, int]]]: return None
        @staticmethod
        def snmp_get_qbridge_fdb(h: str, c: str, timeout: int = 0, retries: int = 0) -> Optional[List[Tuple[str, int, int]]]: return None
        @staticmethod
        def snmp_get_arp_entries(h: str, c: str, timeout: int = 0, retries: int = 0) -> Optional[List[Tuple[str, str, int]]]: return None


import cli_utils # Potrzebne do wywołania cli_get_neighbors_enhanced
import config_loader # Potrzebne do get_communities_to_try
import data_processing
import discovery # Zawiera logikę poszczególnych metod odkrywania
import file_io
...
class NetworkDiscoverer:
    def __init__(self, api_client: LibreNMSAPI, config: Dict[str, Any],
        ...
        self.port_name_to_ifindex_map: Dict[
            Tuple[str, str], Any] = {}
        # config_loader.get_env_config() już tworzy klucz 'cli_credentials'
        # z domyślną strukturą, jeśli nie ma nic w .env
        self.cli_credentials: Dict[str, Any] = config.get('cli_credentials')
    ...
    def _get_cli_credentials_for_device(self, device_info: Dict[str, Any]) -> Optional[Tuple[str, str]]:
        ...
        # Logika wyszukiwania poświadczeń pozostaje taka sama, korzysta z self.cli_credentials
                    ...
                    for val_to_check in identifiers_for_regex:
                        if regex.fullmatch(str(val_to_check)):
                            ...
                            if user and password:
                                ...
                                return user, password
                            break # Przerwij pętlę val_to_check po znalezieniu dopasowania regex
                except re.error as e_re:
                    logger.warning(f"  CLI Creds: Błąd w regex '{pattern_cred}': {e_re}")
                except Exception as e_re_match: # Powinno być bardziej specyficzne jeśli to możliwe
                    logger.warning(f"  CLI Creds: Błąd dopasowania regex '{pattern_cred}': {e_re_match}",
                                   exc_info=False) # Zwykle nie potrzebujemy pełnego tracebacku dla błędów regex
    ...
    def _process_single_target_device(self, target_device_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Pobierz listę community SNMP z konfiguracji
        snmp_communities = config_loader.get_communities_to_try(self.config) # Przekaż cały obiekt config
        ...
        if SNMP_UTILS_AVAILABLE and snmp_communities:
            ...
            # Funkcje w discovery.py przyjmują config
            ...
            device_raw_connections.extend(
                discovery.find_via_snmp_fdb(self.phys_mac_map, target_device_info, snmp_communities, idx_to_name_map, self.config))
            device_raw_connections.extend(
                discovery.find_via_arp_snmp(self.phys_mac_map, target_device_info, snmp_communities, idx_to_name_map, self.config))
        ...
        device_raw_connections.extend(
            discovery.find_via_api_fdb(self.api_client, self.phys_mac_map, target_device_info)) # Ta funkcja nie potrzebuje configu

        # Odkrywanie przez CLI
        enable_cli = self.config.get('enable_cli_discovery', True) # Pobierz flagę z config
        if enable_cli:
            ...
            if cli_creds_tuple:
                ...
                if host_for_cli:
                    if cli_user and cli_pass:
                        ...
                        # cli_utils.cli_get_neighbors_enhanced przyjmuje config
            ...
            logger.info(f"  Odkrywanie przez CLI jest wyłączone w konfiguracji (enable_cli_discovery=False). Pomijam dla {canonical_id}.")
        ...
        for i, device_api_entry in enumerate(self.all_devices_from_api):
            ...
            try:
                ...
                for p_info in ports_for_device:
                    ...
                    if ifindex is None: continue # ifIndex jest kluczowy

                    if_name_raw = str(p_info.get("ifName", "")).strip()
                    if if_name_raw:
                        map_key = (canonical_id_api_dev.lower(), if_name_raw.lower())
                        if map_key in self.port_name_to_ifindex_map and self.port_name_to_ifindex_map[map_key] != ifindex:
                            logger.warning(
                ...
                logger.error(f"  Mapa NazwaPortu->ifIndex: Błąd dla {canonical_id_api_dev} (ID: {dev_id_api}): {e}",
                             exc_info=False) # Zwykle nie potrzebujemy pełnego tracebacku tutaj
    ...
    def _get_ifindex_for_port(self, device_canonical_id: str, port_identifier_raw: Optional[str]) -> Optional[Any]:
        ...
        map_key_to_check = (device_canonical_id.lower(), port_id_lower)
        ...
        if port_id_lower.startswith("ifindex "):
            try: return int(port_id_lower.split("ifindex ")[1])
    ...
    def _enrich_connections(self, raw_connections: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, conn_raw in enumerate(raw_connections):
            ...
            if local_canonical_id == remote_canonical_id and \
               (local_if_raw and remote_if_raw and str(local_if_raw).lower() == str(remote_if_raw).lower()):
                logger.debug(
                    f"    Pominięto (wzbogacanie): Self-connection na tym samym porcie ('{local_canonical_id}':'{local_if_raw}'). Połączenie: {conn_raw}");
                continue

            local_ifindex = conn_raw.get('local_ifindex') # Może być już z LLDP/CDP
            if local_ifindex is None and local_canonical_id:
                local_ifindex = self._get_ifindex_for_port(local_canonical_id, local_if_raw)

            remote_ifindex = conn_raw.get('remote_ifindex')
            if remote_ifindex is None and remote_canonical_id:
            ...
            enriched_conn_data = {
                ...
                "vlan": conn_raw.get('vlan'), # Może być None
            ...
            final_enriched_conn = {k: v for k, v in enriched_conn_data.items() if
                                   v is not None or k in ["vlan", "local_ifindex", "remote_ifindex"]} # Zachowaj vlan, ifindex nawet jeśli None
