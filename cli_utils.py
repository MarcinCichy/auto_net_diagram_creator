...
def _parse_lldp_output(lldp_output: str, local_hostname: str) -> List[Dict[str, Any]]:
    ...
    for block_idx, block_content in enumerate(blocks):
        ...
        if (
                not chosen_remote_port or 'not advertised' in chosen_remote_port.lower() or ':' in chosen_remote_port) and remote_port_desc_val and 'not advertised' not in remote_port_desc_val.lower(): chosen_remote_port = remote_port_desc_val
...
def cli_get_neighbors_enhanced(host: str, username: str, password: str) -> List[Dict[str, Any]]:
    ...
    device_params: Dict[str, Any] = {"device_type": "autodetect", "host": host, "username": username,
                                     "password": password, "global_delay_factor": 5,
                                     "session_log": f"{host}_netmiko_session.log", "session_log_file_mode": "append",
                                     "conn_timeout": 75, "auth_timeout": 90, "banner_timeout": 75}
    ...
    try:
        ...
        try:
            if net_connect.base_prompt: base_prompt_log = net_connect.base_prompt.strip()
        ...
        if base_prompt_log not in ["N/A", "N/A (nie odczytano)", "N/A (błąd)"]:
            if not RE_SIMPLE_PROMPT.fullmatch(base_prompt_log): show_ver_expect_str = base_prompt_log
        if not show_ver_expect_str: show_ver_expect_str = r"[a-zA-Z0-9\S\.\-]*[#>]"  # Ogólny fallback

        try:
            logger.debug(f"  CLI: Próba pobrania 'show version' z {host} (expect_string: '{show_ver_expect_str}')...")
            show_version_params = {"read_timeout": 60}
            if show_ver_expect_str: show_version_params["expect_string"] = show_ver_expect_str
        ...
        if is_extreme_like:
            ...
            cdp_exp_str = RE_SLOT_SYS_PROMPT.pattern
            run_cdp = True
            ...
            try:
                ...
                net_connect.send_command_timing("disable clipaging",
                                                delay_factor=device_params.get('global_delay_factor', 2))
