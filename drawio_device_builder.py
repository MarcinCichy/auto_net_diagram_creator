...
from common_device_logic import (
    ...
    MIN_CHASSIS_WIDTH, MIN_CHASSIS_HEIGHT, PHYSICAL_PORT_LIST_MAX_HEIGHT, LOGICAL_IF_LIST_MAX_HEIGHT
)
...
def _extract_styles_from_template(template_path: str) -> StyleInfo:  # ... (bez zmian) ...
    ...
    if tree is None: logger.warning(f"Nie udało się wczytać szablonu '{template_path}'."); return default_styles
    root = tree.getroot();
    diag_root_cell = root.find(".//root")
    if diag_root_cell is None: logger.warning(f"Brak <root> w szablonie '{template_path}'."); return default_styles
    ...
    for cell in diag_root_cell.findall("./mxCell[@vertex='1']"):
        style, value = cell.get("style", ""), cell.get("value", "")
        if "CHASSIS_TEMPLATE" in value and style:
            loaded_chassis_style = style; logger.info(f"Znaleziono styl obudowy z szablonu Draw.io.")
        elif "PORT_TEMPLATE" in value and style:
            loaded_port_style = style; logger.info(f"Znaleziono styl portu z szablonu Draw.io.")
...
def add_device_to_diagram(
        global_root_cell: ET.Element, device_api_info: Dict[str, Any], api_client: LibreNMSAPI,
        position: Tuple[float, float], device_internal_idx: int, styles: StyleInfo
) -> Optional[Dict[Any, PortEndpointData]]:
    if ET is None: logger.critical("add_device_to_diagram (DrawIO): Moduł ET niedostępny."); return None
    ...
    for row_idx, num_ports_row in enumerate(
        ...
        for col_idx in range(num_ports_row):
            ...
            p_style = drawio_utils.set_style_value(drawio_utils.set_style_value(p_style, "fillColor", fill),
                                                   "strokeColor", stroke)
            p_cell = drawio_utils.create_vertex_cell(p_cell_id, group_cell_id, vis_num_str, px, py, PORT_WIDTH,
                                                     PORT_HEIGHT, p_style);
            ...
            if alias_txt:
                ...
                lbl_unrot_w, lbl_unrot_h = num_lines * LABEL_LINE_HEIGHT + 2 * LABEL_PADDING, max(15, max_len * (
    ...
    if mgmt0_info:
        ...
        mgmt0_cell = drawio_utils.create_vertex_cell(mgmt0_cell_id, group_cell_id, "M", mgmt0_x, mgmt0_y, PORT_WIDTH,
                                                     PORT_HEIGHT, mgmt0_style);
        ...
        if alias_txt_m:
            ...
            lbl_m_w, lbl_m_h = max(30, max_len_m * (
                        LABEL_LINE_HEIGHT * 0.7)) + 2 * LABEL_PADDING, num_lines_m * LABEL_LINE_HEIGHT + 2 * LABEL_PADDING
            aux_sx_m_abs, aux_sy_m_abs = offset_x + mgmt0_x + PORT_WIDTH, offset_y + mgmt0_y + PORT_HEIGHT / 2;
            aux_ex_m_abs, aux_ey_m_abs = aux_sx_m_abs + PORT_ALIAS_LINE_EXTENSION, aux_sy_m_abs
            lbl_drawio_x_m, lbl_drawio_y_m = aux_ex_m_abs + PORT_ALIAS_LABEL_OFFSET_FROM_LINE, aux_ey_m_abs - lbl_m_h / 2
    ...
    dev_id_val, hostname_raw, ip_raw, purpose_raw = dev_info.get('device_id', 'N/A'), dev_info.get('hostname',
                                                                                                   ''), dev_info.get(
        'ip', ''), dev_info.get('purpose', '')
    ...
    if hostname_s and hostname_s != main_name_no_stack and not re.match(r'^\d{1,3}(\.\d{1,3}){3}$',
                                                                        hostname_s): display_extra.append(
        f"Host: {hostname_s}")
    if purpose_s and purpose_s != main_name_no_stack: display_extra.append(f"Cel: {purpose_s}")
    temp_disp_ip = str(ip_raw).strip() if ip_raw and str(ip_raw).strip() else 'N/A'
    if re.match(r'^\d{1,3}(\.\d{1,3}){3}$', hostname_s) and not (
            ip_raw and str(ip_raw).strip()): temp_disp_ip = hostname_s
    base_dev_lbl_html = f"<div style='text-align:left;padding:2px;'><b>{display_name_main}</b>{ports_limit_info_html}<br/>ID: {dev_id_val}"
    if display_extra: base_dev_lbl_html += "<br/>" + "<br/>".join(display_extra)
    base_dev_lbl_html += f"<br/>IP: {temp_disp_ip}</div>"
    phys_ports_html = f"<div style='padding:2px;'><b>Porty Fizyczne ({len(prepared_data.all_physical_ports)}):</b></div><div style='margin:0;padding-left:7px;max-height:{PHYSICAL_PORT_LIST_MAX_HEIGHT}px;overflow-y:auto;overflow-x:hidden;'>"
    ...
    phys_ports_html += "</div>"
    log_ifs_html = f"<div style='padding:2px;'><b>Inne Interfejsy ({len(prepared_data.logical_interfaces)}):</b></div><div style='margin:0;padding-left:7px;max-height:{LOGICAL_IF_LIST_MAX_HEIGHT}px;overflow-y:auto;overflow-x:hidden;'>"
